...
44. `netstat` — Network statistics (use `ss` as a modern alternative).
45. `ifconfig` / `ip` — Configure network interfaces (use `ip` as a modern alternative).
...
100. `cal` — Display a calendar.
101. `man` — Display the manual for a command.
102. `history` — Show previously executed commands.
...
10. **`git reset`**  
    Unstages changes or resets commit state.  
...
### GitHub-Specific Commands (GitHub CLI)
35. **`gh auth login`**  
...
Docker packages applications into portable containers — like shipping containers for software. These commands help build, ship, and run applications consistently across environments.
